...
test('el modo confidencial nunca devuelve una voz de nube', { skip: !hasLocalVoice() }, () => {
  ...
  assert.equal(
    speaker.describe().local,
    true,
    'confidencial con engine=edge debe caer al motor local',
  );
});
...
test(
  'sin motor local, pedirlo falla al construir y no al hablar',
  { skip: hasLocalVoice() },
  () => {
...
test('hasLocalVoice solo promete voz local donde la hay', () => {
  if (!soportado)
    assert.equal(hasLocalVoice(), false, 'no hay motor local fuera de Windows y Linux');
  if (process.platform === 'win32')
    assert.equal(hasLocalVoice(), true, 'Windows siempre trae SAPI');
});
